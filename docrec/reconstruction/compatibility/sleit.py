...
class Sleit(Algorithm):
    ...
    def _lines_mask(self, img, h):
        ...
        assert (img.ndim == 2 or img.dtype == np.bool)
    ...
    def _lines(self, img, t, h):
        ...
        imgs = [img[starts[i] : ends[i] + 1] for i in xrange(starts.size)]
        locations = np.array(map(centroid, imgs)) + starts
    ...
    def _lines_compatibility_x(self, A, B, pixelth, linesth):
        ...
        number_of_compatible_lines = lambda A, B : \
            self._number_of_compatible_lines(A, B, pixelth)
        return lines_difference <= linesth and \
            number_of_compatible_lines(A, B) > 0
    ...
    def _all_pixel_compatibility(self, A, B):
        ...
        return sum(map(pixel_compatibility, range(1, h - 1)))
    ...
    def features(self, t=0.15, d=0):
        ...
        w = np.array(
            [strip.approximate_width() for strip in self.strips.strips]
        )
    ...
    def _compute_matrix(
        self, d=0, t=0.15, h=0.25, p=0.33, linesth=1, blackth=2
    ):
        ''' Compute matrix. '''
        
        tl, tr, pl, pr = self.features(t, d)
                
        # Lines
        left_lines = lambda img: self._lines(img, t, h)
        right_lines = lambda img: self._lines(img, t, h)
        strips_lines = [ \
            StripsLines(
                left_lines(tl[i]), right_lines(tr[i]), pl[i], pr[i]
            )
        for i in range(len(self.strips.strips))]
        ...
        pixelth = p * np.mean(values)
       
        # Compute pairwise distances
        dist = lambda A, B: \
            self._cost_x(
                A, B, pixelth, linesth, blackth
            )
    ...
    def run(self, d=0, t=0.15, h=0.25, p=0.33, linesth=1, blackth=2):
        
        self.matrix = self._compute_matrix(d, t, h, p, linesth, blackth)
        return self
